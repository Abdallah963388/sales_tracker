...
class VisitsRepo {
  ...
  Future<Either<Failure, String>> deleteVisit(int visitId) async {
    try {
      ...
      if (response?.statusCode == 200) {
        final data = response?.data;
        return right(data?['message'].toString() ?? S
                  .of(navigatorKey.currentState!.context)!
                  .deleteSuccessfully,
        );
      } else {
  ...
  Future<Either<Failure, String>> adminDeleteVisit(int visitId) async {
    try {
      ...
      if (response?.statusCode == 200) {
        final data = response?.data;
        return right(data?['message'].toString() ?? S.of(navigatorKey.currentState!.context)!.deleteSuccessfully,
        );
      } else {
        return left(
          ServerFailure.fromResponse(
